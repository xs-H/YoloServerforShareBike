...
import logging
from utils.tts import process_tts_detection, init_tts
...
def args_parser():
    ...
    parser.add_argument("--tts",  type=bool, default=True, help="是否启用 TTS")
    parser.add_argument("--tts-duration",  type=float, default=10, help="TTS 连续检测时间（秒）")
    parser.add_argument("--tts-interval",  type=float, default=30, help="TTS 冷却时间（秒）")
...
def main():
    ...
    try:
        ...
        # 初始化 TTS
        tts_engine = init_tts() if project_args.tts else None
        if project_args.tts and not tts_engine:
            logger.warning("无法初始化语音引擎，语音提醒已禁用")
            project_args.tts = False
        # TTS 状态
        tts_state = {
        ...
        if source.isdigit() or source.endswith((".mp4", "*.avi", '*.mov')):
            ...
            for idx, result in enumerate(model.predict(**yolo_args_dict)):
                ...
                process_tts_detection(result,
                                    project_args.tts,
                                    project_args.tts_duration,
                                    project_args.tts_interval,
                                    tts_engine, tts_state)
